...
wellDF, _, _,  _, _, _ = geomec_classes.Overburden(wellDF, wellinfoDF,name='atividade4 - metodo2', water=0).start()
wellDF, _, _,  _, _, _ = geomec_classes.NormalTensionandGrad(wellDF, wellinfoDF,name='atividade4 - metodo2', sumwater=False).start()
wellDF, top = geomec_classes.Eaton(wellDF, wellinfoDF, name='atividade4 - metodo2', top=36, exponum=3, sumwater=False).start()
wellDF = geomec_classes.Porepressure(wellDF).output()
wellDF = geomec_classes.Unconfinedcompressforce(wellDF).output()
...
geomec_classes.Mudweightwindow(wellDF, name='atividade 4 - metodo2', top=top)